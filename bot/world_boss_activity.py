...
from bot.battle_mode_zone import is_battle_mode_select as _is_battle_mode_select
...
class WorldBossActivity:
    """One participation from a verified Battle Mode hub, returning to that hub."""

    name = "world_boss"
    scope = FlowScope.PER_CHARACTER
    participation_policy = WorldBossParticipationPolicy.ALWAYS_PARTICIPATE
    contract = FlowContract(
        precondition=ComponentRequirement.exact_state(SCREEN_BATTLE_MODE_SELECT),
    ...
    def __init__(
        ...
        *,
        ...
        if not callable(getattr(facts, "read_timer_remaining", None)):
    ...
    def run(self, *, sapphires: int | None = None) -> WorldBossFlowResult:
        ...
        try:
            ...
            if not result.succeeded:
                return result
            # Gameplay terminals (including non-fatal blockers) are clean World Boss.
            before = self.observer.wait_until(
                lambda item: _is_clean_base(item, SCREEN_WORLD_BOSS),
                after_sequence=0, timeout=self.fact_timeout, stable_for=self.stable_for,
            ...
            if not returned.succeeded or not _is_battle_mode_select(returned.final_snapshot):
                return replace(result, status=FlowStatus.FAILED,
                               error=returned.error or "world_boss_hub_return_failed",
    ...
    def _run(self, flow_events: list[FlowEvent], sapphires=None) -> WorldBossFlowResult:
        if self.cancel_requested():
            return WorldBossFlowResult(status=FlowStatus.CANCELLED)
        transitions: list[VerifiedTransitionResult] = []
        battle_modes = self.observer.wait_until(
            _is_battle_mode_select, after_sequence=0, timeout=self.fact_timeout,
