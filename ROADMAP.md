...
L0 prerequisite (2026-09-23): handoff opt-in del board natural en `MonsterWaveActivity`, HIL físico PASS y adquisición G/R1 válida. A1 captura el par fresco antes de OCR; conserva los límites temporales. I2/J2 reemplaza la premisa falsa de rewards exactos por umbrales operativos y drenaje completo. C1 (2026-09-24) mueve capacidad Equipment a la visita Craft: probe fresco Craft→Inventory→Craft, cero slots bloquea antes del primer craft. C2a confirma físicamente una conversión `10 Bronze→2 Silver` y publica en C6b el budget derivado de facts frescos. C2c añade reader productivo local de `TradePanelFact`; C2b cierra intents ActionExecutor para C4/C6b/Treasure/E2.2 y budget Keys lazy desde facts frescos. HIL verificó selector Gold `10(Open)` y OK Gold-full `(0.4996,0.6246)`; no demostró ciclos repetidos de cierre de animación. Los builders C2 quedan pendientes. L sigue no iniciado: no conectar K/J productivamente ni Equipment/Socket Relief aquí.
...
I2/J2 prerequisite → DONE con HIL mínimo y 332/332 regresiones dirigidas (2026-09-23): entrada inclusiva Bronze ≥400, Silver ≥450, Weapon ≥800, Hero ≥800; una visita ya pagada drena Keys/Weapon/Hero hasta su límite operativo (<need), no hasta el umbral. Materials paga Keys aunque Keys esté bajo umbral; Keys no paga General. El resultado determinista de Trading 40→10 sólo decide si Craft rodea Trading; no estima drops MW. Trading es modal a Craft; Treasure es navegación real. Gold-full causal usa un OK verificado por HIL, puede diferir Treasure hasta MW y reintenta el pending una vez. Gold-full no descubierto sin viaje se acepta. R1 fast path rojo del board → DONE local (GT cinco pares rojos, OCR fallback ante ausencia/incertidumbre, Weapon exacto cuando proyección lo exige, evaluator 10/10); umbrales/policy intactos y sin detector global. L permanece no iniciado.
