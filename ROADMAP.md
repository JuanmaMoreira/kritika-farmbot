...
### 7. Integración por presión de recursos — CALLER ADAPTERS NEXT
...
F-Sell standalone → DONE (`equipment_sell_{semantics,reader,policy,operation,runtime}.py`, bulk-only, 284 tests dirigidos, evaluator 12/12, HIL Epic Equipment 120→114). F-composer transversal → DONE offline (`bot/equipment_relief.py`, 25 tests directos; request opaco preservado, contexts secuencialmente frescos, 1 Combine + 1 Sell máximos, candidato explícito obligatorio, sin caller/ruta física nueva). F-caller adapter → **NEXT**, con auditoría/HIL sólo para la ruta concreta no publicada.
