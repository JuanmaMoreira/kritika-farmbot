...
### 6. Inventory Relief simple — SELL STANDALONE DONE

Equipment Sell standalone bulk-only quedó reimplementado desde cero y separado del gameplay: facts OCR por consenso, autorización explícita, grupos Poor–Legendary Equipment/Enhance y Ethereal por tipo, Ethereal+ protegido, un confirm máximo y Item Count decreciente como única postcondición de éxito. HIL autorizado: Epic Equipment `120→114` con una sola venta bulk. No porta chain, planner, thresholds, Karats, scan ni lifecycle experimental. El compositor Combine→retry→Sell permanece pendiente.

### 7. Integración por presión de recursos — NEXT

Conectar la capacidad standalone al caller únicamente cuando un blocker fresco autorice la secuencia causal `Combine → retry caller → Sell autorizado si sigue lleno → retry caller`. No añadir thresholds de presión ni loops especulativos; ventas fallan cerradas ante grado, tipo, Enhance, identidad, grupo o Item Count inciertos.
...
F-Sell standalone → DONE (`equipment_sell_{semantics,reader,policy,operation,runtime}.py`, bulk-only, 284 tests dirigidos, evaluator 12/12, HIL Epic Equipment 120→114). F-composer transversal Combine→retry→Sell → **NEXT**.
