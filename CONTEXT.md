...
Kritika FarmBot 0.2 automatiza tareas por personaje de **Kritika: The White Knights** sobre un dispositivo Android físico. El runtime productivo vigente ejecuta Black Market, World Boss, Monster Wave SKIP-only, Send Stamina, Summon Pet Daily, Daily Quests, Mailbox y Guild Check-In, los compone en sesiones multicharacter y cambia de personaje con `StandardRotation`.
...
`MonsterWaveFlow` es general-purpose Lobby → Lobby y compone `MonsterWaveActivity` hub → hub con `BattleModeZone`. Cada invocación reobserva NEEDS_TICKETS / READY / ACTIVE, realiza como máximo una compra Fill All autorizada y verificada, activa cuando corresponde y ejecuta un único SKIP con MAX fijo. MAX es una acción de dos taps exactos sin verificación intermedia; el frame posterior debe confirmar selección, controles descubiertos, ausencia del tooltip y ACTIVE antes de Start SKIP. No hay Start manual, Auto Battle MW, countdown ni saldo interno de sapphires.

Config mínima vía `.env`: `MW_PURCHASE_SKIP_TICKETS` y `MW_CONTINUE_WHEN_NONBLOCKING_INVENTORY_FULL`, ambas false por defecto. Insufficient sapphires siempre usa No; board usa Yes/No según policy; CLEAR exige OK → MW → Back → hub. Daily observa badge MW en Battle Mode y su ausencia produce `SKIPPED_NOT_ELIGIBLE` sin entrada ni lectura. Sólo Daily exige luego ≥4 sapphires frescos del HUD de MW limpio, después de normalizar entrada y antes de cualquier operación SKIP; 0..3 produce business incomplete y Back verificado. WB/MW consecutivos comparten una visita en el orden seleccionado; el saldo Lobby anterior a WB nunca autoriza MW. Run Selected Flows ignora badge y mínimo Daily, permitiendo MAX con 1–3.

Weekly Results y New Ranking tienen OK → MW limpio confirmado por el usuario y se normalizan automáticamente con reobservación, máximo dos acknowledgements, sin asumir aparición ni orden. Weekly usa su pie fijo, sin season/ranks dinámicos. El literal “The bag is full. Would you like to organize your bag?” corresponde a `popup.socket_inventory_full`. MW reutiliza Socket/Equipment compartidos, sin relief hasta adquirir sus returns: sólo esos blockers duros terminan `MANUAL_RESOLUTION`, detienen sesión/GUI sin cleanup y se proyectan como business incomplete. GOLD insuficiente durante compra sigue sin ground truth específico. Diseño, evidencia y límites en [`docs/MONSTER_WAVE_SKIP.md`](docs/MONSTER_WAVE_SKIP.md).
...
- Monster Wave SKIP-only + patch Weekly/Ranking y guard Daily ≥4 listo para revisión en `feature/monster-wave-skip`: **1925/1925 hardware-free** en 292,48 s y **438 dirigidos** verdes. Evaluator global: **510 frames, 0 wrong, 0 ambiguous**; 45.816 pares reutilizados y 1.614 evaluados sin invalidaciones ni relajación de thresholds. 97 frames curados, 16 assets runtime y 108 raws de tres adquisiciones preservados; OCR MW validado con 27 positivos y 52 negativos contextuales. `git diff --check`, UTF-8, enlaces, whitespace nuevo y hashes válidos. Live previo del usuario confirmó ACTIVE/MAX/Start y board false → No; este patch no requirió nuevo hardware. Sin commit ni push; cambios ajenos preservados. Ver `docs/MONSTER_WAVE_SKIP.md`.
...
Revisar Monster Wave SKIP-only y patch Weekly/Ranking + guard Daily ≥4 en `feature/monster-wave-skip`, sin commit ni push. World Boss y MW comparten zona preparada, orden explícito y una visita para posiciones consecutivas. Las tres adquisiciones raw se preservan. Follow-ups: GOLD insuficiente al comprar tickets y returns de relief MW; Tower, Arena, loops, RoutineSpec y nuevos reliefs siguen fuera. Ver `docs/MONSTER_WAVE_SKIP.md`. Identity y Rotation conservan sus contratos.
