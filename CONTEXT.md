...
- **Resource Route Planner I-impl + executor J (post-V1):** `plan_resource_route` permanece puro y determinista; `MonsterWaveResourceRouteRuntime` consume un plan ya construido exactamente una vez. Unknown/contradictory no emiten input y `NO_PREREQUISITES` devuelve el snapshot MW corriente. Craft y el único bloque Trading se serializan mediante MW como anchor: Craft siempre vuelve por Back a MW fresco antes de Trading; Keys precede a Materials dentro de la misma visita Trading; éxito exige X/Back y snapshot MW final fresco. No hay auto-replan, Equipment Relief proactivo, MW SKIP/batalla/reward ni routing genérico.
...
- Trading Center: C1-C6b están cerrados. `KeysPromotionRuntime` ejecuta C6a con facts frescos y consume únicamente `SILVER_TO_GOLD + OUTPUT_FULL`: preserva `PendingCausalOperation`, posee drain+retry y acepta un plan pequeño de navegación source-aware. Standalone conserva `Trading→Lobby→Treasure→Quick Menu→Trading`; J aporta `Trading X→MW→Quick Menu→Treasure→Back→MW→Quick Menu→Trading`. Ambos restauran Avatar & Keys, releen filas y reintentan la misma operación una sola vez. Un segundo `OUTPUT_FULL` falla cerrado; no hay estimación de capacidad, `UP_TO(n)`, Equipment Inventory, relief ni scroll;
...
- Regla de routing: Quick Menu conserva sólo el origen inmediato y no tiene destino Monster Wave. J usa MW como anchor de prerrequisitos: `MW→Craft→Back→MW` antes de un bloque Trading posterior; C6b MW hace `Treasure→Back→MW` antes de reabrir Trading. Keys+Materials permanecen agrupados dentro del mismo bloque Trading. Nunca se depende de un segundo nivel de origen ni se reconstruye MW desde Lobby;
...
J está CLOSED offline y deja un plan supplied ejecutado una vez con snapshot MW final fresco. El siguiente frente de producto es K standalone/debuggable, separado de este checkpoint; no se inició K ni L.
