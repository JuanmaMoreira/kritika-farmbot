...
**Estado:** rediseño híbrido 0.2 — Fase 3H.2 completada; cierre semántico mínimo del flow Black Market multi-character cubierto
...
- El cierre semántico mínimo queda productivo para `screen.character_select`, `screen.lobby`, `menu.quick`, `screen.black_market` y `popup.purchase_confirmation`. Dentro de Black Market todavía falta el problema separado de identificar identidad/tipo y slot de cada oferta; precio/recurso sólo se añadirá si evidencia funcional lo exige. No se eligió OCR, templates, detector entrenado ni VLM para ese problema.
