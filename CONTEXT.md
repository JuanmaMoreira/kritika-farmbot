...
- Inventory Relief Chain general, Craft o adapters de caller. El compositor transversal existe, pero ningún caller productivo lo consume todavía; una integración debe aportar reconocimiento Equipment Full, ruta verificada a Combine/Inventory y retorno fresco. Sell exige un `EquipmentSellRequest` caller-supplied con autorización+candidato explícitos;
...
El compositor transversal causal quedó cerrado offline sin caller productivo ni ruta física nueva. Siguiente frente exacto, en tarea separada: elegir un primer caller y auditar/adquirir sólo sus rutas caller→Combine, caller→Equipment Inventory y retornos antes de construir su adapter; el caller deberá aportar el `EquipmentSellRequest` preautorizado. No iniciar Craft ni Monster Wave desde este checkpoint.
