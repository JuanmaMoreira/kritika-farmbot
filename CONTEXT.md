...
La suite normal permanece hardware-free y contiene 402 tests verdes. La arquitectura híbrida completa todavía no está implementada. Fases 3D y 3G quedan cerradas con el pipeline `ScrcpyFrameSource → PerceptionEngine → ContextResolver` validado live, sin gameplay ni acciones. `landmark.lobby_commerce_pair` permanece como alternativa offline y Battle Mode Select necesita evidencia más diversa o una señal con separación más amplia antes de entrar a Perception productiva.
