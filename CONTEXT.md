...
`SummonPetDailyFlow` es `PER_CHARACTER`, usa el id `_daily` `summon_pet_daily` y declara `screen.lobby → screen.lobby`. Daily ausente es success/no-op. Daily presente entra a Summon y elige Epic sólo con `status.pet_epic_available`; si observa unavailable usa Premium sin distinguir ticket de GOLD. Ambas rutas ejecutan exactamente `1 (Open)`, exigen `screen.pet_summon_result` estable y sólo completan después de cerrar el resultado y confirmar la desaparición estable de la Daily.
...
`popup.insufficient_gold` usa la rama segura existente `No`, emite un business event no fatal y deja la Daily pendiente. Pet Full usa `Yes`, exige `screen.pet_combine` e invoca `PetSummonSpaceRelief` una sola vez por `run()`. `RELIEVED` autoriza un único retry fresco; `NO_RELIEF_AVAILABLE` y un segundo Pet Full producen resolución manual no fatal, `FAILED` es fallo técnico y `CANCELLED` se propaga.

`PetSummonSpaceRelief` es una support operation separada con precondición `screen.pet_combine` y outcomes `RELIEVED`, `NO_RELIEF_AVAILABLE`, `FAILED` y `CANCELLED`. Primero intenta Combine All; sólo si no libera espacio busca por páginas bounded `candidate.pet_low_tier` Normal/Rare, verifica el mismo tier en Mass Evolve y ejecuta `Cancel` antes de cambiar de tab. Luego abre como máximo un batch Epic mediante `10 (Open)`, acepta de forma bounded un resumen único o hasta diez resultados secuenciales, vuelve a Combine y hace un segundo Combine All. El efecto se verifica mediante `screen.pet_combine_result` + `TapThroughAnimation` + retorno estable. No hardcodea costo de runas. Venta, slots, candidatos Epic+, tiers ambiguos y estrategias Legendary/Ethereal están ausentes por diseño; el bloqueo sin salida es `NO_RELIEF_AVAILABLE`, no fallo técnico.
...
- Temporal: `setting.auto_battle = ON/OFF/UNKNOWN`; sólo OFF inequívoco y contexto de batalla vigente autorizan toggle. ON se verifica en una ventana fresca posterior de 10 muestras con budget bounded de 6 s, ajustado tras dos timeouts productivos idénticos en 9/10 muestras dentro del budget anterior de 4 s.
...
- Suite hardware-free: **1244/1244 tests verdes**.
...
- La adquisición HIL de Pet Combine cubrió Combine All con efecto/no-effect, runas Epic llenas desde dos ramas, candidatos Normal/Rare sin identidad, selección/cancelación, confirmaciones Mass Evolve con tier explícito, resultado compartido, aperturas Epic unitarias y el ciclo extremo que termina conceptualmente en `NO_RELIEF_AVAILABLE`. No se implementó ninguna support operation.
...
- Pet Summon/Pet Combine sólo tiene evidencia de una temporada/render. La disponibilidad Epic depende del render del juego y tolera costos promocionales, pero no se leyó cantidad. La búsqueda low-tier está calibrada para la grilla 4×4 observada y permanece bounded por páginas; el soft block extremo sólo tiene criterio de parada, no resolución. La adquisición original verificó resultados Epic unitarios; la nueva apertura única `10 (Open)` tolera por contrato un resumen o resultados secuenciales, pero su presentación exacta requiere el smoke productivo mínimo.
- `main.py` y módulos legacy preservados no son entrypoints productivos. `AdsManager` continúa standalone y desacoplado.

## Próximo trabajo

`SummonPetDailyFlow` y `PetSummonSpaceRelief` están integrados y no se avanza a Arena. Venta de pets, compra de slots, Legendary/Ethereal y resolución destructiva del soft block permanecen fuera de alcance.
