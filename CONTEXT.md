...
La semántica productiva reconoce `screen.guild`, `status.guild_attendance_active`, `status.guild_attendance_completed` y el status independiente `status.guild_attendance_daily_active`. Attendance deriva del valor HSV de una franja interior del botón: pendiente es rojo brillante (`V 167.16–168.41`) y completado es oscuro/presionado (`V 81.89–84.58`). La ROI queda fuera del bubble transitorio y la lectura de ambos estados del botón se suprime mientras `menu.quick` está abierto. El flow vigente todavía no consume el nuevo status Daily.
