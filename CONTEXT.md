...
El primer smoke de la espera pasiva confirmó `initial_wait=65`, polling bounded y tolerancia de `UNKNOWN`, pero agotó la ventana porque el completion predicate exigía erróneamente `screen.world_boss_battle + overlay` aunque el resolver ya preservaba overlays con base no resuelta. Se retiró ese gating sólo del consumidor y se mantuvo intacta la calibración productiva: 8/8 positivos Raid Complete, 0 FP/FN y gap raw `0,564908`. La telemetría DEBUG registra por poll sequence, estado base, overlays, detección y confidence semántica; el resumen agrega último estado, overlays, sequence, máximo confidence, polls y `UNKNOWN`. No se añadió burst multiframe, captura automática, detector de red ni ping; el live PASS de este refuerzo sigue pendiente del usuario.
