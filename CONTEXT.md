...
- **Monster Wave board G + R1 (post-V1):** reader demand-driven y snapshot descriptivo puros, fuera de la orquestación MW. El popup muestra cinco pares `balance/límite mostrado` por identidad y orden: Brawler's Badges, Weapon Material, Hero Weapon Material, Bronze Key y Silver Key. R1 prueba presión dura por rojo sólo en el par numérico y omite OCR cuando el número exacto no afecta la ruta; ausencia/incertidumbre usa OCR. Weapon conserva OCR para proyección determinista si Hero no está rojo. Dos samples concordantes y frescos producen un fact; el snapshot exige contexto/popup coherentes y barrera de secuencia. Sin detector global ni cambio de umbrales I2/J2. Gold-key capacity, reward quantities, recetas y necesidades de materiales no se derivan del board.
...
- **Craft standalone (post-V1):** `enter()` desde Equipment Inventory exige ≥1 slot libre; la rama J que llega desde MW entra a Craft y ejecuta C1 antes del primer Craft: Quick Menu→Inventory, Item Count fresco por consenso, Back→Craft fresco. Cero slots se expone como `EQUIPMENT_CAPACITY_BLOCKED`, sin relief. Cada operación Hero `ONE`/`MAX_AVAILABLE` conserva guards de receta, coste, no premium y decremento exacto. I2 decide visitar Craft por presión/proyección sin requerir receta ni capacidad de otra pantalla; J drena hasta <49 con presupuesto y sin repetir el probe durante la misma visita Craft.
- **Rotation y sesión:** ejecución multi-flow/multicharacter, cancelación, eventos, reporte y selección desde GUI/CLI.

## Validación vigente

- R1 2026-09-23: GT humano de panel con cinco pares rojos, tres capturas pasivas ADB; negativos normales de Weapon/Hero/Bronze/Silver en tres frames G. Board/planner/J/K dirigidos **75/75**; evaluator board incremental **10/10**. OCR evitado por instrumentación: cinco llamadas menos en panel all-red; Weapon conserva lectura exacta cuando Hero es normal. Sin suite global ni input al juego.
...
- C1 2026-09-24: probe de capacidad por visita Craft en J; HIL manual previo confirmó Craft→Quick Menu→Inventory `86/112`→Back→Craft limpio en el mismo personaje. Código exige Item Count concordante y retorno Craft fresco, bloquea cero slots antes de `MAX_AVAILABLE` y no invoca relief. I2 deja receta/capacidad como facts de ejecución; **64 tests directos y 824 afectados** verdes. Sin evaluator ni full suite: no cambió percepción ni infraestructura productiva amplia. Wiring L sigue pendiente.
- C2a 2026-09-24: HIL de una sola conversión Bronze→Silver, personaje no saturado en Trading/Keys: Bronze `40/10→30/10`, Silver `64/10→66/10`, popup `1/20`, un tap Trade autorizado y ACK limpio sin `OUTPUT_FULL`. C6b publica el rendimiento observado `10 Bronze→2 Silver` y `make_budget(FreshKeyFacts)` acota promociones por Bronze disponible más Silver inicial/generable. C2 builders y L1 siguen pendientes.
- C2c 2026-09-24: `TradingPanelReader` local produce `TradePanelFact` desde frames Trading resueltos y frescos para C4/C6b: OCR de identidad, `have/need`, cantidad y fronteras; panel sin segundo coste probado por región vacía, otros layouts fail-closed. Replays reales 15/15 y regresiones C4/C6b/Keys/C3 144/144.
- C2b 2026-09-24: C4/C6b/E2.2 aceptan input semántico despachado por `ActionExecutor`; `TreasureRuntime` despacha x1/x10 por ese límite y el final de E2.2 conserva `DismissTreasureResult`. HIL físico confirmó `10(Open)` Gold 45→35 sin Karat y OK Gold-full `(0.4996,0.6246)` con Trading/Keys limpio; el cierre/ciclo repetido de animaciones no se infiere del HIL del selector. `KeysPromotionRuntime.run()` deriva el budget sólo después de `FreshKeyFacts` mediante el único `make_budget`; J ya no exige un entero anticipado. `TradingPanelFact` sigue siendo de C2c. Composición C2 y L1/L2 pendientes.
...
K permanece standalone/debug. L0-A entrega un board causal opt-in; el default mantiene Yes/No y la sesión se detiene cerrada si el handoff no tiene consumidor. I2 ya no exige rewards entrantes exactos: los drops probabilísticos no intervienen en la ruta. C1 obtiene capacidad Equipment durante la visita Craft; L stage-ready no se inició y falta la composición productiva del caller.
