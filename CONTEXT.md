...
**Estado:** rediseño híbrido 0.2 — primer vertical slice de Fase 4 implementado y validado live sobre un personaje
...
- La suite normal contiene 357 tests hardware-free. No cambiaron `SEMANTIC_CONFIDENCE_THRESHOLD`, reglas de contexto, source weighting, hysteresis ni temporalidad. Battle Mode Select, taps/element learning, OCR, VLM, ActionExecutor, SessionRunner y gameplay permanecen deferred. Al cierre de 3F, Fase 3D todavía dependía de repetir live el smoke completo en 3G.

La Fase 3G revalidó el repair 3F en hardware real y cerró el smoke pendiente de 3D, sin acciones Android ni cambios productivos:

- El baseline previo reprodujo 357 tests hardware-free, materializó los cinco frames Workbench curados y obtuvo nuevamente 62/62 estados end-to-end, 28 `UNKNOWN`, cero `AMBIGUOUS` y cero wrong. Black Market quedó 11/11 y Purchase Confirmation 6/6.
- `--compare-once` recibió el mismo ndarray BGR `uint8` `(1224, 2712, 3)` en ambos paths. Los raw scores Lobby/Character Select/Black Market/Purchase fueron respectivamente `0.989405`, `0.060530`, `0.014615` y `0.063203`; el input conservó su hash, `input_unchanged=True` y `raw_scores_equal=True`.
- El smoke live current-season procesó 1478 snapshots en `494.75 s`, sequences `1 → 29407` y frame `2712×1224`. Lobby, Character Select y la reentrada a Lobby resolvieron establemente; Battle Mode Select permaneció `UNKNOWN` sin base ni overlay.
- Black Market reparado resolvió `screen.black_market` sin Lobby, Character Select ni `AMBIGUOUS`: en la ventana marcada sin popup su raw permaneció aproximadamente `0.398–0.400` y semantic confidence `0.992–1.000`. Con el popup visible conservó esa base y añadió `popup.purchase_confirmation`; el máximo raw del prompt fue aproximadamente `0.997–0.999`, con confidence `1.000`.
- Guild Shop sin popup permaneció `UNKNOWN`. Al abrir allí el mismo prompt produjo `UNKNOWN + popup.purchase_confirmation`, con raw aproximadamente `0.996–0.997` y confidence `1.000`, confirmando que el overlay continúa desacoplado de Black Market. El tooling vigente reporta el máximo raw combinado del detector, no los scores individuales de sus dos variantes.
...
- `datasets/quick_menu_evidence_manifest.json` conserva 18 positivos y 15 negativos seleccionados. Excluye eventos `interaction.before/after`, GT inicial/transicional atrasado y frames dudosos; `9485/9546` se reclasificaron visualmente como negativos porque el menú ya estaba cerrado. Inventory permanece candidate y se mapea a base productiva `UNKNOWN`.
- `menu.quick` se promovió como overlay/menu global dentro del slot existente de `ResolvedState`, no como base ni como categoría nueva. `landmark.quick_menu_lobby_tile` usa un crop current-season de 126×140 y una región relativa suficientemente amplia para las dos posiciones observadas, sin offsets dependientes del contexto.
...
- `AGENTS.md` contiene ahora como fuente canónica el protocolo human-in-the-loop. En hardware, chat + `steer` es el canal principal; navegación, confirmaciones y ground truth se solicitan semánticamente por chat, mientras Workbench queda limitado a instrumentación explícita. La regla se integró antes de la primera prueba interactiva y se aplicó durante toda la fase.
- El baseline inicial fue exactamente `803aaf2`, working tree limpio y 374 tests hardware-free verdes. No se hizo push.
- Quick Menu quedó revalidado live post-promoción mediante capturas pasivas: Lobby cerrado produjo Lobby raw `0.984854`, Quick raw `0.252197` y `screen.lobby`; abierto produjo Lobby raw `0.091039`, Quick raw `0.987894` y `UNKNOWN + menu.quick`; cerrado otra vez produjo Lobby raw `0.987541`, Quick raw `0.244844` y `screen.lobby`. No hubo `AMBIGUOUS`, los frames conservaron igualdad entre paths, age `125–156 ms` y cleanup completo.
- La afirmación inicial de grid 4×2 fue corregida por evidencia y confirmación humana: Black Market actual y legacy tienen 5 filas × 2 columnas, 10 ofertas. Legacy define `buy-item-01`…`buy-item-10`, dos franjas verticales para `assets/ui/gold-coin-bm.png` y ningún detector KARATS específico; no existe flow Black Market implementado en el código preservado. Se reutilizaron grid, asset GOLD y secuencia semántica, no targets/taps/coupling legacy.
- La policy cerrada es exactamente `BUY iff currency == GOLD` y `NEVER BUY KARATS`. No se reconoce identidad de items, no se leen precios, balance GOLD ni balance KARATS, y Video/Purchase Complete/ausencia se tratan conservadoramente como no elegibles.
- `BlackMarketGoldDetector` precarga el asset legacy `55×57` y mide diez regiones estrechas derivadas geométricamente del grid 5×2. Sólo emite `currency.black_market.gold` por encima de confidence `0.80`; `Observation.value` conserva el índice row-major y `region` la franja de currency. No crea contextos ni targets de acción.
- `datasets/black_market_currency_manifest.json` conserva ocho configuraciones/estados visualmente revisados, dos human-confirmed live, con 80 labels internos: 24 GOLD, 45 KARATS y 11 Video/Purchased. La evaluación agrega 840 regiones negativas de pantallas no Black Market: 24 TP, 0 FP, 0 FN y específicamente 0 FP sobre KARATS. Los raw positives son `0.934380/0.952117/0.961620` min/median/max, el máximo negativo global es `0.573178` y el gap `0.361202`.
- Hay positivos reales en siete posiciones row-major (`0,2,3,4,5,6,7`) y negativos en las diez. Las posiciones `1,8,9` no tienen todavía positivos empíricos; tests sintéticos insertan el mismo asset nativo en cada una de las diez regiones para validar geometría, pero esa distinción permanece documentada.
- El popup adicional mencionado y la respuesta a falta de oro resultaron ser el mismo estado. Seleccionar directamente una oferta GOLD inalcanzable muestra `Insufficient Gold. Would you like to purchase Gold?`; no pasa antes por `popup.purchase_confirmation`. Elegir No cierra el popup y retorna a Black Market estándar. La rama Yes no se exploró porque podría iniciar una compra de oro.
- Se promovió `popup.insufficient_gold` como overlay independiente mediante `landmark.insufficient_gold_prompt` y un asset current-season `375×52`. Un positivo human-confirmed frente a 95 negativos obtuvo raw `0.999978` contra `0.443980`, gap `0.555998`, 1 TP y 0 FP/FN. La base subyacente permanece `screen.black_market` y Purchase Confirmation conserva semántica separada.
...
## Primer vertical slice — implementado y validado
...
`BlackMarketFlow` tiene scope `PER_CHARACTER`, comienza en `screen.lobby` y navega directamente `Lobby → Black Market → Lobby`. Black Market no se abre desde Quick Menu. El flow analiza una sola vez los diez slots del grid 5×2, toma los detectados como GOLD y los recorre en orden row-major. Una compra exitosa exige que el mismo slot quede `Purchased`. La policy es `BUY iff currency == GOLD` y `NEVER BUY KARATS`; no identifica item ni lee precio o balance y no usa OCR.
...
Una primera lectura con cero slots GOLD es un resultado normal: se registra de forma simple y el flow termina success/no-op después de volver a Lobby. Puede deberse a juego manual, tienda ya procesada, ausencia de ofertas GOLD u otra condición normal indistinguible por percepción. Los logs iniciales no incluyen slot, item, precio, balance ni identidad de personaje.

La policy inicial de debug distingue outcomes esperados —Purchase Confirmation, Insufficient Gold, cero GOLD y compra verificada mediante `Purchased`— de errores técnicos o estados inesperados. Ante estos últimos: log → cleanup seguro → abortar el proceso completo, sin recovery sofisticado. `purchase_unverified` no afirma éxito ni permite continuar si el slot esperado no aparece como `Purchased` dentro del timeout.

Los contratos concretos nuevos son `RuntimeSnapshot` (observations, resolved state, runtime facts y geometría del mismo frame), espera bounded que rechaza frames stale, intents semánticos tipados y `ActionExecutor` como único traductor a taps ADB. La entrada a Black Market exige estabilidad visual durante `0.75 s`, porque el título aparece antes de que termine de cargar la grilla; no se añadió voting ni memoria al resolver. Los intents cubren abrir/cerrar Black Market, seleccionar slots `0..9`, aceptar Purchase Confirmation y rechazar Insufficient Gold. Toda conversión final usa coordenadas normalizadas y dimensiones de `frame.shape`, nunca `wm size`.

`BlackMarketPurchasedDetector` usa dos crops nativos curados del texto `Purchase Complete!` sobre las diez regiones de precio. El manifest contiene 10 frames y 100 labels Black Market. Tras integrar tres compras live confirmadas humanamente, Purchased obtiene 11/11 TP, 0 FP y 0 FN frente a 929 negativos; raw positivo mínimo/mediana/máximo `0.881583/0.942529/0.999968`, máximo negativo `0.557578` y gap `0.324005`. GOLD conserva 25/25 TP, 0 FP/FN y cero FP sobre 61 KARATS; el positivo live amplió cobertura empírica al slot 8.

La validación hardware fue incremental. El probe de entrada leyó GOLD inicial `[6,8]` sin seleccionar ofertas. El one-slot smoke compró el slot 6 mediante Purchase Confirmation → Yes y verificó `Purchased`; la confirmación humana incorporó ese frame al dataset. El full smoke posterior leyó GOLD `[7,8]`, intentó sólo esos dos slots, verificó ambos como `Purchased`, no encontró Insufficient Gold y terminó success. Una captura pasiva posterior fue confirmada humanamente e incorporada al manifest. Finalmente el target Close Black Market devolvió a un frame fresco `screen.lobby`, cerrando el ciclo del personaje. Todos los sources hicieron cleanup; un primer intento del full smoke falló en `adb get-state` antes de cualquier tap porque el daemon no pudo iniciar dentro del aislamiento y el reintento autorizado funcionó.

`RotationStrategy`, `rotation.standard`, `SessionPlan` y `SessionRunner` siguen sin implementar. OCR y VLM permanecen deferred hasta que un caso funcional los requiera. Quick Menu no participa en `BlackMarketFlow`.
...
La suite normal permanece hardware-free y contiene 460 tests verdes. La arquitectura híbrida completa todavía no está implementada porque faltan Rotation y orquestación de sesión. `landmark.lobby_commerce_pair` permanece como alternativa offline y Battle Mode Select necesita evidencia más diversa o una señal con separación más amplia antes de entrar a Perception productiva.
