...
- Trading Center: C1/C2 readiness/materials, C3 row facts, C4 trade verificado y C5 Avatar & Keys permanecen cerrados. El prerrequisito físico de C6b también está DONE: `TradingRuntime` compone Lobby→Trading directo, restauración idempotente/fresca de Avatar & Keys y cierre X→Lobby mediante tres acciones HIL-calibradas, single-attempt, sin scroll ni trades. Smoke productivo 2026-09-21: entrada/salida fresh PASS; rama General→Avatar & Keys fresh PASS; C6b continúa NO INICIADO;
- E2 Treasure (post-V1, hot-context): runtime autónomo + fast Gold drain cerrados físicamente de punta a punta. HIL productivo 2026-09-21: Gold Keys 27→0, initial open SUCCESS, frontera Karat fresca, cero taps derechos posteriores, premium 33,509→33,509, dos taps `TapThroughAnimation` en el único target profile-owned `(0.08,0.65)`, overlay cerrado y Treasure estable por GT humano. El harness produjo un falso negativo sobre el frame estable 707 porque trataba `treasure_gold_key_selector` aislado como retorno de Gold; el postcondition se corrigió offline para exigir ausencia de `RIGHT_GOLD_OPEN_MAX`, con regresión dedicada. Sin wiring a sesión productiva y C6b NO INICIADO;
