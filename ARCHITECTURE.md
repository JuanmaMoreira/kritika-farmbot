...
`quick_menu_accessible` es una capability de policy (`bot/quick_menu.py`), no `screen.quick_menu_accessible` ni un nodo de navegación. El allow-list productivo contiene `screen.lobby` y `screen.world_boss`; ambos abrieron/cerraron live el mismo `menu.quick` con el target común del header. Nuevos contextos se incorporan sólo después de esa validación. Quick Menu actúa como hub operacional para la normalización mínima a Lobby, sin grafo ni pathfinding.
