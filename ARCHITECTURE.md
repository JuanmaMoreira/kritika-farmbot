...
Pet Combine añade sólo semántica para una futura support operation. `PetLowTierCandidateDetector` recorre la grilla visible fija, emite `candidate.pet_low_tier` con región y valor `normal`/`rare` únicamente ante frames verdes/azules inequívocos, y se calla en modo de selección o bajo modal; identidad, Epic+ y ambigüedad no son candidatos. `popup.pet_mass_evolve_confirmation` es un estado único cuyo landmark lleva el tier explícito. `popup.pet_epic_runes_full` se reutiliza tanto desde Combine All como desde Mass Evolve. Combine All y Mass Evolve comparten `screen.pet_combine_result` y la actividad existente `activity.combine_animation_tappable`; el base dedicado evita autorizar taps desde `UNKNOWN`. Perception no decide candidato, páginas, estrategia ni outcome.
...
`SummonPetDailyFlow` es `PER_CHARACTER`, usa el id `summon_pet_daily`, declara Lobby → Lobby y posee únicamente la policy Daily. Ausencia del badge es no-op; presencia autoriza Epic cuando su status está disponible y Premium sólo cuando Epic está unavailable. El flow siempre solicita `1 (Open)` y no decide ticket contra GOLD. Resultado estable + cierre + desaparición estable de Daily es completion. Insufficient GOLD es un business event con Daily pendiente. Pet Full delega una vez a `PetSummonSpaceRelief`; relief positivo habilita un solo retry, mientras unavailable o un segundo Pet Full terminan en resolución manual no fatal. Failure y cancelación de la operación se propagan.
...
`PetSummonSpaceRelief` consume la semántica Pet Combine sin mover policy hacia Perception. Exige Combine limpio, intenta Combine All y, si no libera, busca páginas bounded sólo por `candidate.pet_low_tier` Normal/Rare. La selección deriva del centro de la región observada; Mass Evolve exige confirmación del mismo tier y `Cancel` antes del cambio de tab. La etapa Epic ejecuta como máximo un `10 (Open)` cuando el status sigue disponible, exige al menos un resultado observable y cierra de forma bounded un resumen o hasta diez resultados secuenciales. Después vuelve a Combine para un segundo Combine All. Efecto requiere resultado tappable y retorno estable mediante `TapThroughAnimation`; no material o el soft block terminan en `NO_RELIEF_AVAILABLE`. Nunca vende, compra slots, elige Epic+ ambiguo ni usa Legendary/Ethereal.
