...
Pet Summon separa observación de policy. `PetEpicAvailabilityDetector` clasifica el render brillante/apagado de la tarjeta Epic y emite disponibilidad mutuamente excluyente sin leer el contador; se calla fuera de Summon y bajo selectors/popups. Los detectores Premium sólo distinguen ticket/GOLD y sus menús porque cambian la interacción, pero no eligen recurso. El resultado común exige dos landmarks estructurales —banner dorado y pergamino— que no dependen de identidad o rareza. Los popups de fragmentos, GOLD insuficiente y Pet Full permanecen outcomes observables; Pet Combine se resuelve como destino mediante shell + tab activo + prompt. Ninguno ejecuta navegación, summon o cleanup.
...
La semántica Pet Summon tampoco tiene consumidor funcional todavía. El futuro `SummonPetDailyFlow` deberá poseer la policy Daily y Epic → Premium; Perception no debe decidir prioridad ni ticket contra GOLD. `INSUFFICIENT_GOLD` será un `FlowEvent` no fatal con Daily pendiente. Pet Full podrá delegar a una support operation `PetSummonSpaceRelief`, pero el flow y la semántica adquirida sólo conocen el popup, la transición `Yes` y `screen.pet_combine`; venta, slots, Mass Evolve y cualquier estrategia de limpieza quedan fuera de este contrato.
