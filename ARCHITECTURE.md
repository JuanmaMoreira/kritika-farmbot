...
`MonsterWaveActivity.run(yield_resource_board=True)` entrega el popup de recursos que aparece naturalmente después de `StartMonsterWaveSkip` como `FlowStatus.RESOURCE_BOARD_PENDING`, con `board_sequence` del postestado fresco. No emite Yes/No ni Back después de reconocerlo; el caller recibe el popup abierto y debe adquirir G desde esa barrera. El default `False` conserva la respuesta configurada Yes/No. `ProductiveMonsterWaveFlow` (L1, composition root `_build_monster_wave`) consume ese handoff: primera actividad con el request original + `yield=True`, adquisición A1 una vez tras `board_sequence`, planner I2 una vez, cierre + J una vez como máximo, salida a Battle Mode una vez y resume causal del MISMO request + `yield=False` una vez; plan vacío resume sin fabricar trabajo y sin replan post-J. Si un caller opt-in entrega el handoff a `SessionRunner` sin consumirlo, la sesión falla cerrada con `resource_board_pending`, sin cerrar la zona, ejecutar otro flow ni rotar.

I2 separa **entry policy** de **drain policy**: los umbrales deciden si se paga el viaje; dentro de una capacidad se consume todo el trabajo disponible y seguro, aunque el balance caiga bajo el umbral. El GT de drops probabilísticos por sapphire no requiere reward entrante exacto. Gold capacity sigue no observable: si ningún umbral paga Trading, Gold-full puede quedar sin descubrir y se acepta porque no bloquea MW por sí solo. Sólo `Silver→Gold OUTPUT_FULL` inicia Treasure. El display rojo al llegar/superar el límite UI queda para una futura optimización perceptiva separada; I2/J2 usa números existentes. L1 productivo consume el pending con bounds 1/1/1/1; L2 (relief Equipment/Socket) sigue fuera: un blocker en el resume se expone sin aliviarlo.
...
C2b permite a C4/C6b/E2.2 despachar controles semánticos por `ActionExecutor`; C4 conserva las verificaciones del panel, C6b exige alert Gold-full fresco y Keys limpio tras un único OK, y Treasure conserva guards Gold/Karat y postcondiciones. `TreasureRuntime` ya envía x1/x10 por intents existentes; el botón derecho del resultado E2.2 usa intent propio y el cierre final usa `DismissTreasureResult`. HIL verificó el selector `10(Open)` Gold 45→35 y el OK Gold-full en `(0.4996,0.6246)`; no prueba un ciclo selector→cierre de animación→selector. J puede omitir el presupuesto inicial: `KeysPromotionRuntime` obtiene `FreshKeyFacts`, llama `make_budget` una vez por operación bounded y devuelve el remanente para el retry diferido. El presupuesto sólo limita pasos; C6a/C6b siguen decidiendo con facts nuevos. C2 composition root permanece pendiente.
