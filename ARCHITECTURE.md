...
Lobby no es un hub obligatorio. Trading no expone Quick Menu mientras está abierto y siempre debe cerrarse por su X roja antes de otra ruta. En el camino Lobby→Trading de `TradingRuntime`, la X vuelve a Lobby; HIL C6b mostró que Trading abierto desde Quick Menu conserva Treasure debajo y su X vuelve a Treasure. Treasure sí expone Quick Menu y `QuickMenuTradingRuntime` posee el camino específico Treasure→Quick Menu→Trading con postcondición Trading fresca. Craft y Monster Wave también tienen acceso físico a Quick Menu para integración futura, pero no se enrutan aquí: el principio es usar Quick Menu directo cuando el origen lo permite y el destino está verificado, y sólo usar el close/return propio cuando el contexto lo exige.
