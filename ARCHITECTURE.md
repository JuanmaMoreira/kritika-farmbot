...
`resource.sapphires` conserva su extractor por defecto de Lobby/Survival para WB. El contexto explícito `screen.monster_wave` selecciona una variante del HUD MW con ROI propia y requisito de cero overlays; no hay fallback al extractor Lobby. Ambas lecturas exigen consenso de dos muestras frescas en hasta tres observaciones separadas 0,20 s. La composición Daily habilita el mínimo MW de 4 después de normalizar entrada, antes de tickets/activación/MAX/Start, y la activity reobserva estado SKIP después del OCR. Saldo 0..3 retorna al hub con `monster_wave.daily_sapphires_below_minimum`; lectura inconclusa falla conservadoramente. El wrapper manual no aplica ese mínimo. No hay cache de saldo ni accounting entre WB/MW.

`CharacterContext` contiene la clase reconocida en `name` y su score OCR en `name_confidence`, ambos opcionales. El índice `1..N` es posición de sesión, no identidad. Recursos, stamina, timers y rank son Runtime Facts. `LobbyNameRecognizer` observa exclusivamente el nombre personal del HUD en Lobby limpio y aplica un mapping Unicode exacto de 28 nombres a clases, con una sola línea y score mínimo 0,95. Si falta la clave exacta, sólo acepta cuatro variantes completas verificadas de `一/二/三` mediante una tabla inmutable separada del ground truth; no repara caracteres ni compara substrings. Ambas ramas conservan el mismo threshold y retornan Unicode canónico. La ausencia, baja confianza o excepción conserva `CharacterContext()` y `Character N`.

`ProductiveRuntime.run_session` conecta `character_context_factory` al runner. Se llama una vez después de la primera precondición, reutilizando el snapshot del probe existente; no agrega capturas, navegación ni inputs. El candidato se invalida antes de cada probe y se libera al consumirlo o terminar la sesión. Un primer requisito que termine fuera de Lobby conserva fallback. `session.character.started` mantiene su orden y nombre ausente; los eventos posteriores con campo de nombre y el resultado reciben la clase. SessionReport proyecta ese nombre o `Character N`, y GUI/CLI sólo renderizan. No hay policy de identidad en resolver, flows ni Rotation. Diseño, corpus y límites en [`docs/CHARACTER_IDENTITY_V1.md`](docs/CHARACTER_IDENTITY_V1.md).
...
`status.world_boss_daily_active` tiene consumidor externo en `WorldBossDailyEligibility` sólo para sesiones Daily; la activity y el wrapper standalone siguen siendo generales. `GuildCheckInFlow` permanece sin el guard `status.guild_attendance_daily_active` hasta adquirir el negativo discriminante pendiente.
...
`MonsterWaveActivity` ejecuta exactamente un SKIP con MAX desde el hub y verifica retorno al hub; `MonsterWaveFlow` compone la misma activity con apertura/cierre Lobby. `MonsterWaveDailyEligibility` observa sólo el badge de su card, sin navegar; el wrapper manual no aplica Daily. La policy MW tiene sólo compra de tickets y aceptación del board. Activación, compra, MAX y Start SKIP dependen de evidencia fresca; no existen facts session-wide de activación ni saldos simulados. `SelectMonsterWaveMax` representa dos taps consecutivos intencionales dentro de ActionExecutor, sin observación intermedia; el activity verifica después ACTIVE + selección MAX + controles descubiertos y ausencia del tooltip. Contrato completo en [`docs/MONSTER_WAVE_SKIP.md`](docs/MONSTER_WAVE_SKIP.md).

La entrada MW normaliza Weekly Results y New Ranking mediante intents OK distintos y postcondición fresca de desaparición del popup. Máximo dos acknowledgements, sin orden combinado supuesto ni taps desde UNKNOWN/overlays desconocidos. Son progreso operacional sin business event rutinario; MANUAL_RESOLUTION queda reservado a los blockers duros cuyos returns de relief no están adquiridos.

`FlowStatus.MANUAL_RESOLUTION` y `SessionStatus.MANUAL_RESOLUTION` expresan una detención de negocio sin postcondición de retorno adquirida: el runner conserva eventos/posición y termina sin cerrar zona ni rotar. GUI lo presenta explícitamente y SessionReport como BUSINESS_INCOMPLETE. No significa éxito hub → hub, cancelación ni fallo técnico. MW no invoca Socket/Equipment relief hasta adquirir un return propio; no se amplían los contracts de esas support operations.
...
`SessionPlan` expresa `character_count`, flows `PER_CHARACTER` ordenados y `RotationStrategy`, con Eligibility opcional por posición. `ProductiveRuntime.run_session` vincula World Boss y Monster Wave a `PreparedActivity` y una `BattleModeZone` compartida. El runner asegura Lobby al abrir visita, observa el precheck opcional y lo conserva pendiente sin bloquear la apertura; después verifica el hub y evalúa Eligibility. NOT_ELIGIBLE descarta el precheck; UNKNOWN/error conserva precedencia técnica; sólo ELIGIBLE (o ausencia de check) permite consumirlo antes de ejecutar la activity hub → hub. Un skip conserva su posición. Sólo posiciones consecutivas de la misma instancia de zona comparten visita; el último cierre verifica Lobby antes del siguiente flow o Rotation. UNKNOWN/error abortan sin cleanup a ciegas. Registry y ejecución manual usan el wrapper general Lobby → Lobby, sin Daily. No se amplía el allow-list de Rotation. Daily eligible abre Battle Mode una vez. Contratos en [`docs/BATTLE_MODE_SHARED_ZONE.md`](docs/BATTLE_MODE_SHARED_ZONE.md). Para cada personaje, `SessionRunner`:

1. asegura sólo el requisito del siguiente componente;
2. prepara la zona cuando corresponda, evalúa el check opcional y ejecuta u omite cada posición en orden;
3. verifica la postcondición, cierra la zona al terminar el tramo consecutivo y registra los resultados;
